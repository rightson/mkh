 ...
 *   node fallback.js --force       # 即使已存在也用網站版覆寫
 *
...
function toMarkdown(ep) {
  ...
  lines.push('source_type: transcript-site'); // 第三方逐字稿站（備援來源）
  lines.push('---');
  lines.push('');
...
async function main() {
  ...
  if (opts.help) {
    console.log('用法: node fallback.js [--out <dir>] [--force]');
    return;
  }
